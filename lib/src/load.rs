...
pub fn get_wordlist<'a, R: Rng + Sized>(path: Option<PathBuf>, delimiter: Option<&str>, rng: &mut R) -> VecStringResult<'a> {
    let wordlist = match path {
        Some(path) => load_wordlist(&path, &delimiter.unwrap_or("\n"), rng)?,
